 ...
 *  */
function jtpl_function_html_pagelinks_custom($tpl, $action, $actionParams, $itemsTotal, $offset, $pageSize = 15, 
                                      $paramName = 'offset', $displayProperties = array())
{
    ...
    if ($itemsTotal > $pageSize) {
        ...
        $defaultDisplayProperties = array('start-label' => '|&laquo;',
                                          'prev-label'  => '&laquo;',
                                          'next-label'  => '&raquo;',
                                          'end-label'   => '&raquo;|',
                                          'area-size'   => 0);
        ...
        for($curidx = 0; $curidx < $itemsTotal; $curidx += $pageSize) {
            if($offset >= $curidx && $offset < $curidx + $pageSize) {
                $pages[$numpage] = '<li class="pagelinks-current"><a href="#">'.$numpage.'</a></li>';
        ...
        if (!empty($displayProperties['start-label'])) {
            ...
            if ($prevBound >= 0) {
                ...
                echo ' pagelinks-disabled"><a href="#">',$displayProperties['start-label'], '</a>';
            }
            echo '</li>', "\n";
        }
        ...
        if (!empty($displayProperties['prev-label'])) {
            ...
            if ($prevBound >= 0) {
                ...
                echo ' pagelinks-disabled"><a href="#">',$displayProperties['prev-label'], '</a>' ;
            }
            echo '</li>', "\n";
        }
        ...
        if (!empty($displayProperties['next-label'])) {
            ...
            if ($nextBound < $itemsTotal) {
                ...
                echo ' pagelinks-disabled"><a href="#">',$displayProperties['next-label'], '</a>' ;
            }
       	    echo '</li>', "\n";
        }
        ...
        if (!empty($displayProperties['end-label'])) {
            ...
            if ($nextBound < $itemsTotal) {
                ...
                echo ' pagelinks-disabled"><a href="#">',$displayProperties['end-label'], '</a>' ;
            }
       	    echo '</li>', "\n";
        }
