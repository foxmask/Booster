...
<table class="records-list">
    <thead>
        <tr><th>{@boosteradmin~admin.items_list.type_name@}</th><th>{@boosteradmin~admin.items_list.name@}</th><th>{@boosteradmin~admin.items_list.date_modified@}</th><th>{@boosteradmin~admin.item_by@}</th></tr>
    </thead>
{foreach $datas_mod as $data}
    <tbody>
        <tr><td>{$data->type_name}</td><td><a href="{jurl 'boosteradmin~items:editmod',array('id'=>$data->id)}">{$data->name}</a></td><td>{$data->modified|jdatetime}</td><td>{$data->nickname}</td></tr>
    </tbody>
{/foreach}
</table>
{else}
